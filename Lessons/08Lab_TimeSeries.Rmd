...
temperature_date_plot <- ggplot(BR_complete, aes(x = Date, y = Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)
print(temperature_date_plot)

rainfall_date_plot <- ggplot(BR_complete, aes(x = Date, y = Rainfall))+
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)
print(rainfall_date_plot)

#it's not looking like there are any data gaps in either variable
#can also do this by doing summary(BR_complete$Rainfall), if there are any NA values they would show up as a new separate column in the report it spits out. In assignment need to create a new factor that goes from first to last date, do some kind of join and something will show up as NA because there is no observation recorded in one of the variables even though the other one is there so it will generate as an NA. Can do inner join or left join, this will make the NAs show up if they are present. 
...
#the trend is looking very seasonal, rainfall is pretty much totally consistent, but temperature_C has been seeing a slight increase generally over time series

rainfall_temp_together <- plot_grid(temperature_date_plot, rainfall_date_plot, nrows = 2, ncol = 1)
print(rainfall_temp_together)

#doing it this way so you can see better the results
...
Temperature_ts <- ts(BR_complete$Temperature_C, start = c(1901, 1), frequency = 12)
...
Temperature_decomposed <- stl(Temperature_ts, s.window = "periodic")
...
Data_components_rainfall <- as.data.frame(Rainfall_decomposed$time.series[,1:3])

Data_components_temperature <- as.data.frame(Temperature_decomposed$time.series[,1:3])

Data_components_rainfall <- mutate(Data_components_rainfall, observed = BR_complete$Rainfall, date = BR_complete$Date)

Data_components_temperature <- mutate(Data_components_temperature, observed = BR_complete$Temperature_C, date = BR_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series

trend_plot_rainfall <- ggplot(Data_components_rainfall, aes(x = date, y = trend))+
  geom_line()

print(trend_plot_rainfall)

trend_plot_temperature <- ggplot(Data_components_temperature, aes(x = date, y = trend))+
  geom_line()
print(trend_plot_temperature)

together3 <- plot_grid(trend_plot_rainfall, trend_plot_temperature, ncol = 1,  nrow = 2)
print(together3)

#thats how i did it but i think thats not right, Luana does below 

ggplot(Data_components_rainfall, aes(x=date))+
  geom_line(aes(y=observed))+
  geom_line(aes(y=trend), color = "red")+
  geom_line(aes(y=seasonal), color = "blue")

ggplot(Data_components_temperature, aes(x = date))+
  geom_line(aes(y=observed))+
  geom_line(aes(y=trend), color = "red")+
  geom_line(aes(y=seasonal), color = "blue")
#that still looks weird so idk i think i did it right 
