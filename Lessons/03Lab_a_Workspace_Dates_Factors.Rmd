...
#lubridate is part of the 'tidyverse' thing that just some guy made
...
fix.early.dates <- function(d, cutoff) {
       ...
       year(d) <- ifelse(m > cutoff, 1900+m, 2000+m)  #have to define cutoff
...
#Alternative 1
date_obj_past <- fast_strptime(str_past,"%y-%b-%d",cutoff_2000=24L) #the L at the end here just makes it stored as an integer, long integer, instead of as a float value aka has a decimal
date_obj_past
...
#Convert to a factor #what is thiss....
...
sites$dow <- factor(
  ...
  labels=c("Sun","Mon","Chewsday","Wed","Thu","Fri","Sat"))
