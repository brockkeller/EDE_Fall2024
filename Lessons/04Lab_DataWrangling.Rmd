...
# dplyr filtering 
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #that makes sense just tell it
...
NTL.phys.data.1999 <- 
  filter(NTL.phys.data, year4 == "1999")

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday.Lake.1990.to.1999 <- 
...
> Answer: we want to filter on the data itself, not on how the data is organized becasue the row numbering could be arbitrary and kind of shifts and rearrange anytime we move data around. Just 0 sense.
...
Tuesday.Lake.90s.only.July <-
  filter(NTL.phys.data.Tuesday.Lake.1990.to.1999, month(sampledate) == 07)
...
surface.july <- Tuesday.Lake.90s.only.July %>% 
  filter(depth == 0) %>% 
