...
regression.pm.ozone <-
  lm(formula = Ozone ~ PM2.5, data = EPAair)

print(regression.pm.ozone)

summary(regression.pm.ozone)

#p value is <2.2e-16 and R-squared is 0.1314. thats right
...
EPAair.subset <- EPAair %>% 
  select(mean_Lat:Ozone) %>% #only selecting the columns from lat to ozone because they are the numeric ones which means we can use them for lin regression
  na.omit()
...
Ozonebest.reg <- lm(data=EPAair,
                    PM2.5 ~ Ozone + Month +mean_Lng + mean_Lat)
summary(Ozonebest.reg)

#now overall the R-squared value has increased from 0.13 to 0.19, meaning the multiple variables explain variance in ozone better than just whatever the single first one we used could
