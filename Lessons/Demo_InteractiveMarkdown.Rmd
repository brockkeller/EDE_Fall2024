...
date: "Fall 2024"
output: html_document #changed from pdf to htlm
runtime: shiny #added this for some reason
editor_options: 
...
my_plot <- nutrient_data %>% 
  ...
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
  scale_shape_manual(values = c(17, 15)) + #to manually set a shape, can look up the codes online or ask GPT to do it
  labs(
...
the_parameter <- 'tn_ug'
start_year = 1990
end_year = 1998 #by just setting the date parameters here we can easily change the plot instead of fussing in the code itself

#just copied pasted from the code chunk above
my_plot <- nutrient_data %>% 
  filter(year4 > start_year & year4 < end_year) %>% 
  ggplot(aes(
    x = sampledate,
    y = tp_ug, #sadly cannot just say y = the_parameter because it's like not in the environment
    #instead can do y = .data[[the_parameter]] 
    # y = !!sym(the_parameter) also works for some reason but idk don't think it matters
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
plot_it <- function(the_parameter, start_year, end_year){ #doing it this other weird way, can see it in environment tab
#then can play around with it in console for example just say plot_it('nh34', 1995, 2000) and it automatically makes that

#took out last lines and the thing nutrient_data was pointing to in 135
#just copied pasted from the code chunk above
nutrient_data %>% 
  filter(year4 > start_year & year4 < end_year) %>% 
  ggplot(aes(
...
#### Other widgets
Shiny has many other input widgets: https://shiny.rstudio.com/gallery/widget-gallery.html.THIS WEBSITE IS COOL AND SHOWS YOU THE CODES TO ADD WIDGETSBelow we'll use a Select Box widget to specify the variable shown in our plot. We'll also add a slider range to limit the date shown in our plot. 
...
renderPlot( #something something find from the interactivemarkdown_completed doc to see how he did it
  plot_it(
