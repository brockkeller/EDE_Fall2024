...
Date  | 2018-2019
Source | EPA Air Quality System (ASQ)
...
```{r project.setup, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.cap= "Ozone by Month 2018", fig.height= 3}
epa_data$Date <- ymd(epa_data$Date)

Data_2018 <- epa_data %>% 
  filter(year(epa_data$Date) == 2018) %>% 
  drop_na(Ozone)
...
Ozone_by_month_2018 <-
  Data_2018 %>% 
  ggplot(aes(x = factor(month(Data_2018$Date), levels = 1:12, labels = month.abb), y = Ozone))+
  geom_boxplot()+
  labs(title = "Monthly Ozone concentrations (2018)", x = "Month", y = "Concentration (ug m^3)")

print(Ozone_by_month_2018)

```


```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.cap= "Ozone by Month 2019", fig.height= 3}
Data_2019 <- epa_data %>% 
  filter(year(epa_data$Date) == 2019) %>% 
  drop_na(Ozone)

Data_2019$Date <- ymd(Data_2019$Date)

Ozone_by_month_2019 <-
  Data_2019 %>% 
  ggplot(aes(x = factor(month(Data_2019$Date), levels = 1:12, labels = month.abb), y = Ozone))+
  geom_boxplot()+
  labs(title = "Monthly Ozone concentrations (2019)", x = "Month", y = "Concentration (ug m^3)")

print(Ozone_by_month_2019)
...
## Git Repository
[mylink](https://github.com/brockkeller/EDE_Fall2024)
